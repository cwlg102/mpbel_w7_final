...
#현재파일 : 
CT_folder_path = 'D:\CodeMaster\w7_dataset\P001_HN1_CT_2018-11-01_102652_RT^01.RT.Head.Neck.(Adult)_Head.&.Neck..3.0..B31s_n150__00000'
RTst_folder_path = 'D:\CodeMaster\w7_dataset\P001_HN1_RTst_2018-11-01_102652_RT^01.RT.Head.Neck.(Adult)_re_n1__00000'
######################################import CT info############################################
filename_list = []
fileidx = []
...
for ctrs in range(len(all_ctr)):
    ...
    for idx, _slice in enumerate(ctr_volume_coord):
        for point_idx in range(len(_slice)):
            ...
            dx = [1, -1, 0, 0]
            dy = [0, 0, 1, -1]
            voxelnp[int(zi)][int(yi)][int(xi)] = np.array([round((R/255)*65535), round((G/255)*65535), round(B/255)*65535])
            queue.append((xi, yi))
            while queue:
                x, y = queue.popleft()
                for i in range(4):
                    ...
                    if std_distance <= this_distance:
                        ...
                        if (nx, ny) == (xiplus, yiplus):
                            break
                        queue.append((nx, ny))
                        voxelnp[int(zi)][int(ny)][int(nx)] = np.array([round((R/255)*65535), round((G/255)*65535), round(B/255)*65535])
