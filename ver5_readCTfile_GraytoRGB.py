...
#CT폴더 불러오기
folder_path = basepath + '\P081_HN81_CT_2019-10-11_092543_RT^01.RT.Head.Neck.(Adult)_Head.&.Neck..3.0..B31s_n149__00001'
#Contour파일 불러오기
ctr_voxel = np.load('D:\CodeMaster\w7voxels\p081rtst\p081voxel_ctr.npy')
...
superior_ct = sorted_CT_file[0]

convert_coeff = 2**(superior_ct.BitsAllocated-1)

if type(superior_ct.WindowCenter) == pydicom.valuerep.DSfloat:
    window_center = superior_ct.WindowCenter
    window_width = superior_ct.WindowWidth
else:
    window_center = superior_ct.WindowCenter[1]
    window_width = superior_ct.WindowWidth[1]
...
CT_voxel[CT_voxel >= np.max(CT_voxel)] = 2**superior_ct.BitsAllocated-1
...
np.save('D:\CodeMaster\w7voxels\p081ct\p081CTvoxel', CT_ctr_voxel)

 
for k in range(len(CT_ctr_voxel)):
    write_png('D:\CodeMaster\w7voxels\p081ct\p081ct%d.png' %(k+1), CT_ctr_voxel[k])
