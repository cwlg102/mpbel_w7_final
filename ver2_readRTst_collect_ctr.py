...
#현재파일 : 
CT_folder_path = 'D:\CodeMaster\w7_dataset\P091_HN91_CT_2019-06-10_173337_._Neck.3.0_n155__00000'
filename_list = []
fileidx = []
...
folder_path = 'D:\CodeMaster\w7_dataset\P091_HN91_RTst_2019-06-10_173337_._OAR91_n1__00000'
pic = []
...
voxelnp = np.zeros((155, 512, 512, 3))
    ...
    for idx, _slice in enumerate(ctr_volume_coord):
        for point_idx in range(len(_slice)):
            ...
            try:voxelnp[round(_slice[point_idx][2])][round(_slice[point_idx][1])][round(_slice[point_idx][0])] = \
                np.array([round((R/255)*65535), round((G/255)*65535), round(B/255)*65535])
                ##################|기준좌표|/resolution -> 넘파이에 넣을 좌표에 더할값##########################  
            except:break
...
np.save('D:\CodeMaster\w7voxels\p091rtst\p091voxel_ctr', voxelnp)
